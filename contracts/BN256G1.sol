...
/**
 * This library is a modified version of https://github.com/witnet/bls-solidity/
 * The changes are
 *   - use `staticcall` instead of `call` when invoking the EC precompiles,
 *     which allows those functions to be `view`
 *   - correct the pairing docstrings so they list the imaginary components first,
 *     and specify that the pairings have to multiply to 1 (ie. the first term is inverted)
 *     This change will be obsolete by https://github.com/witnet/bls-solidity/pull/3
 *     if it is merged
 */
...
library BN256G1 {
  ...
  function add(uint256[4] memory input) internal view returns (uint256, uint256) {
    bool success;
  ...
  function multiply(uint256[3] memory input) internal view returns (uint256, uint256) {
    bool success;
    uint256[2] memory result;
    assembly {
